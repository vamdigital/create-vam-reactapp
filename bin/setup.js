...
import fs, { appendFileSync, unlinkSync } from "fs";
...
let isVSCode = false;
const repo = "https://github.com/vamdigital/create-vam-reactapp.git";
const execPromise = promisify(exec);

console.log(
  "💿 Welcome to create-vam-react. Let's get you set up with a new project."
);
...
async function setup() {
  ...
  try {
    ...
    fs.readFile(indexFilePath, "utf-8", function (readFileError, data) {
      ...
      let newValue = data.replace(/Vite App/gi, `${folderName}`);

      fs.writeFile(indexFilePath, newValue, "utf-8", function (writeFileError) {
